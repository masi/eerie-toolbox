 ...
 */
PageWalker.prototype.processQueue = function(queue, onLoad, onDone, onLoadError, onError) {
	if (this.i !== undefined) {
	...
	this.i = 0;
	this._processQueueItem(onLoad, onDone, onLoadError, onError);
};
 ...
 * @protected
 */
PageWalker.prototype._processQueueItem = function(onLoad, onDone, onLoadError, onError) {
	var self = this,
		context, url;
	if (this.i === undefined) {
	...
	if (this.queue[this.i]) {
		...
		this.client.open(url)
			...
			.then(function() {
				self.i++;
				self._processQueueItem(onLoad, onDone, onLoadError, onError);
			})
